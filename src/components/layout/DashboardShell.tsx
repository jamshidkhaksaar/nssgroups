...
export default function DashboardShell({
  ...
  notificationsCount = 0, notifications = [], userName, userRole,
}: DashboardShellProps) {
  ...
  const displayName = userName ?? 'Portal User';
      ...
      <div className="flex">
        ...
        {hasSidebar && (
          <aside className={`sticky top-0 hidden h-screen shrink-0 flex-col border-e border-[rgba(var(--gold-rgb),0.10)] bg-[rgba(var(--bg-rgb),0.60)] backdrop-blur-xl transition-all duration-300 lg:flex ${sidebarOpen ? 'w-64' : 'w-16'}`}>
            ...
            <button onClick={() => setSidebarOpen((v) => !v)} className="flex items-center gap-2 border-t border-[rgba(var(--gold-rgb),0.10)] px-4 py-3 text-xs text-[rgba(var(--text-rgb),0.45)] transition-colors hover:text-[rgb(var(--gold-rgb))]">
              ...
              {sidebarOpen && <span className="nss-mono uppercase tracking-wider">{t('dashboard.collapse')}</span>}
            </button>
          </aside>
        )}
        {/* Main column */}
        <div className="flex min-h-screen min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-40 flex items-center gap-3 border-b border-[rgba(var(--gold-rgb),0.12)] bg-[rgba(var(--bg-rgb),0.92)] px-4 py-2.5 backdrop-blur-md sm:px-6">
            ...
            <button onClick={toggleTheme} className="rounded-lg p-2 text-[rgba(var(--text-rgb),0.5)] transition-colors hover:bg-[rgba(var(--gold-rgb),0.08)] hover:text-[rgb(var(--gold-rgb))]" title={theme === 'dark' ? t('dashboard.themeLight') : t('dashboard.themeDark')}>
                ...
                <>
                  ...
                  <div className="absolute end-0 top-full z-50 mt-1 w-80 overflow-hidden rounded-xl border border-[rgba(var(--gold-rgb),0.15)] bg-[var(--panel)] shadow-xl">
                    <div className="flex items-center justify-between border-b border-[var(--card-border)] px-4 py-2.5">
                      <span className="nss-mono text-[10px] uppercase tracking-[0.16em] text-[rgba(var(--text-rgb),0.45)]">{t('dashboard.alerts')}</span>
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length === 0 ? (
                        <p className="px-4 py-8 text-center text-xs text-[rgba(var(--text-rgb),0.45)]">{t('dashboard.noAlerts')}</p>
                      ) : (
                        notifications.map((n) => (
            ...
            <div className="relative">
              <button onClick={() => setUserMenuOpen((v) => !v)} className="flex items-center gap-2 rounded-lg border border-[rgba(var(--gold-rgb),0.15)] bg-[rgba(var(--bg-rgb),0.4)] py-1 pe-2 ps-1.5 transition-colors hover:border-[rgba(var(--gold-rgb),0.35)]">
                <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[rgba(var(--gold-rgb),0.15)] text-xs font-bold text-[rgb(var(--gold-rgb))]">{displayName.charAt(0)}</div>
                <div className="hidden text-start sm:block">
                  <div className="text-xs font-semibold leading-tight text-[rgb(var(--text-rgb))]">{displayName}</div>
                  <div className="nss-mono text-[9px] uppercase tracking-wider text-[rgba(var(--text-rgb),0.45)]">{displayRole}</div>
                </div>
                ...
                <>
                  ...
                  <div className="absolute end-0 top-full z-50 mt-1 w-52 overflow-hidden rounded-xl border border-[rgba(var(--gold-rgb),0.15)] bg-[var(--panel)] py-1 shadow-xl">
                    <div className="border-b border-[var(--card-border)] px-4 py-2.5">
                      <div className="text-sm font-semibold text-[rgb(var(--text-rgb))]">{displayName}</div>
                      <div className="nss-mono text-[10px] uppercase tracking-wider text-[rgba(var(--text-rgb),0.45)]">{displayRole}</div>
                    </div>
                    ...
                    <button className="flex w-full items-center gap-2 px-4 py-2 text-sm text-[rgba(var(--text-rgb),0.7)] transition-colors hover:bg-[rgba(var(--gold-rgb),0.06)] hover:text-[rgb(var(--gold-rgb))]" onClick={() => { setUserMenuOpen(false); resetPortalData(); }}>
                      <RotateCcw className="h-4 w-4" /> {t('dashboard.resetData')}
                    </button>
                    <button className="flex w-full items-center gap-2 px-4 py-2 text-sm text-[rgba(var(--text-rgb),0.7)] transition-colors hover:bg-[rgba(var(--gold-rgb),0.06)] hover:text-[rgb(var(--gold-rgb))]" onClick={() => { setUserMenuOpen(false); logout(); navigate('/'); }}>
          ...
          <main className="flex-1 p-4 sm:p-6 lg:p-8">
            ...
            <CommandDialog open={paletteOpen} onOpenChange={setPaletteOpen} title={t('dashboard.quickNav')} description={t('dashboard.commandDescription')}>
              <CommandInput placeholder={t('dashboard.commandPlaceholder')} />
              <CommandList>
                <CommandEmpty>{t('dashboard.noResults')}</CommandEmpty>
                <CommandGroup heading={t('dashboard.nav')}>
                  <CommandItem onSelect={() => go('/')}>
                    <Home className="h-4 w-4" /> {t('nav.home')}
                    <CommandShortcut>↵</CommandShortcut>
                  </CommandItem>
                  <CommandItem onSelect={() => go('/admin')}>
                    <LayoutDashboard className="h-4 w-4" /> {t('nav.adminPortal')}
                  </CommandItem>
                  <CommandItem onSelect={() => go('/client-portal')}>
                    <LayoutDashboard className="h-4 w-4" /> {t('nav.clientPortal')}
                  </CommandItem>
                  <CommandItem onSelect={() => go('/partner-portal')}>
                    <LayoutDashboard className="h-4 w-4" /> {t('nav.partnerPortal')}
                  </CommandItem>
                </CommandGroup>
