  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md border-[var(--card-border)] bg-[var(--panel)] text-[rgb(var(--text-rgb))]">
        <DialogHeader>
          <DialogTitle className="text-lg font-bold flex items-center gap-2 text-[rgb(var(--gold-rgb))]">
            <MapPin className="w-5 h-5 text-[rgb(var(--gold-rgb))]" />
            {t('portal.checkpoint.dialogTitle')}
          </DialogTitle>
          <p className="text-xs text-[rgba(var(--text-rgb),0.6)]">
            {t('portal.checkpoint.shipmentLabel')} <span className="nss-mono text-[rgb(var(--text-rgb))]">{order.trackingNumber}</span> ({order.origin} → {order.destination})
          </p>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2 text-sm">
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-[rgba(var(--text-rgb),0.6)] font-medium">{t('portal.checkpoint.updateStatusLabel')}</label>
          ...
          {/* Location Input */}
          <div className="space-y-1.5">
            <label className="text-xs text-[rgba(var(--text-rgb),0.6)] font-medium">{t('portal.checkpoint.locationLabel')}</label>
            <Input
              placeholder={t('portal.checkpoint.locationPlaceholder')}
              value={location}
              onChange={(e) => setLocation(e.target.value)}
              className="bg-[rgba(var(--text-rgb),0.03)] border-[var(--card-border)] text-[rgb(var(--text-rgb))]"
            />
          </div>

          {/* Status Headline */}
          <div className="space-y-1.5">
            <label className="text-xs text-[rgba(var(--text-rgb),0.6)] font-medium">{t('portal.checkpoint.headlineLabel')}</label>
            <Input
          ...
          {/* Detailed Notes */}
          <div className="space-y-1.5">
            <label className="text-xs text-[rgba(var(--text-rgb),0.6)] font-medium">{t('portal.checkpoint.notesLabel')}</label>
            <Textarea
          ...
          <div className="space-y-2 pt-2 border-t border-[var(--card-border)]">
            <span className="text-xs font-semibold text-[rgba(var(--text-rgb),0.6)] flex items-center gap-1">
            ...
            <div className="space-y-1.5 max-h-28 overflow-y-auto pe-1">
              {order.checkpoints.map((chk) => (
                <div key={chk.id} className="text-xs p-2 rounded bg-[rgba(var(--text-rgb),0.03)] border border-[var(--card-border)]">
                  <div className="flex justify-between text-[rgb(var(--text-rgb))] font-medium">
                    <span>{chk.location}</span>
                    <span className="text-[10px] text-[rgba(var(--text-rgb),0.45)] nss-mono">{new Date(chk.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <div className="text-[11px] text-[rgb(var(--gold-rgb))]">{chk.status}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          <Button variant="ghost" className="text-[rgba(var(--text-rgb),0.6)] hover:bg-[rgba(var(--text-rgb),0.05)] hover:text-[rgb(var(--text-rgb))]" onClick={onClose}>
            {t('portal.checkpoint.cancelBtn')}
          </Button>
          <Button className="bg-[rgb(var(--gold-rgb))] text-[#1d1233] hover:bg-[rgba(var(--gold-rgb),0.88)] font-bold" onClick={handleSubmitCheckpoint}>
